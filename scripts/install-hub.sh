...
generate_env() {
  ...
  if [ -f "$env_file" ]; then
    ...
    SONDE_API_KEY=$(sed -n 's/^SONDE_API_KEY=//p' "$env_file" 2>/dev/null || echo "")
    if [ -n "$SONDE_API_KEY" ]; then
      info "Preserving existing API key from .env"
    fi
  fi

  if [ -z "${SONDE_API_KEY:-}" ]; then
    SONDE_API_KEY=$(generate_key)
    info "Generated new API key."
  fi

  cat > "$env_file" <<EOF
# Sonde Hub Configuration — generated by install-hub.sh
SONDE_API_KEY=${SONDE_API_KEY}
...
# ── Summary ──────────────────────────────────────────────────────────
print_summary() {
  local key_preview="${SONDE_API_KEY:0:12}..."
  ...
  echo "  API Key:    ${YELLOW}${SONDE_API_KEY}${RESET}"
